...
2-Nov-2022     AS          Initial
****************************************************************************/
TySifiveTraceDecodeError SifiveDecoderInterface::Decode(char* out_file, uint32_t start_msg_num, uint32_t start_msg_offset)
{
	if(out_file == nullptr)
	{
	...
	if (sf_name != nullptr) {
		...
		if (pf_name != nullptr) {
			...
			trace = new (std::nothrow) Trace(tf_name,ef_name,numAddrBits,addrDispFlags,srcbits,od_name,freq,start_msg_offset);
...
//	should look at source code display!
	...
	do {
		...
		if (ec == TraceDqr::DQERR_OK) {
			if(msgInfo)
			{
				msg_num = msgInfo->msgNum;
				msg_offset = msgInfo->offset;
			}
			if(profile_flag)
			{
...
TySifiveTraceDecodeError SifiveDecoderInterface::DecodeBuffer(char* out_file, char* p_buff, const uint32_t size, uint32_t start_msg_num, uint32_t start_msg_offset)
{
	if(out_file == nullptr)
	{
		return SIFIVE_TRACE_DECODER_INPUT_ARG_NULL;
	...
	do {
		if (sim != nullptr) {
			...
			if (ec == TraceDqr::DQERR_OK)
			{
				if(msgInfo)
				{
					if(msgInfo->offset < m_trace_start_idx || msgInfo->offset > m_trace_stop_idx)
					{
						//printf("\nSkipping");
						out_to_file = false;
					}
...
TySifiveTraceDecodeError SifiveDecoderInterface::Configure(const TDecoderConfig& config)
{
	...
	start_msg_offset = config.start_msg_offset;

	return SIFIVE_TRACE_DECODER_OK;
}
void SifiveDecoderInterface::GetMsgNumAndOffset(uint32_t &msg_num, uint32_t &msg_offset)
{
	msg_num = this->msg_num;
	msg_offset = this->msg_offset;
}
void SifiveDecoderInterface::SetMsgNumAndOffset(uint32_t msg_num, uint32_t msg_offset)
{
	this->msg_num = msg_num;
	this->msg_offset = msg_offset;
	trace->SetMsgNumAndOffset(msg_num, msg_offset);
}
